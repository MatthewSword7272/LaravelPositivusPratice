...
const CaseStudies = () => {
  ...
  return (
    <>
      ...
      <Slider {...settings} className={"lg:!hidden caseStudies"}>
        {caseStudies.map((text, index) => (
          <div key={index} className={"bg-dark rounded-4xl px-12 py-10"}>
            <p className={"text-white mb-5 leading-tight"}>{text}</p>
            <Link style={"simple_green"}>Learn more</Link>
          </div>
        ))}
      </Slider>
